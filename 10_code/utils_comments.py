...
def submatrix(arr):
    ...
    x, y = np.nonzero(
        arr
    )  # returns two arrays: x contains the row indices of non-zero elements, and y contains the column indices of non-zero elements.
...
def init_patch_square(image_size, patch_size):
    ...
    noise_dim = int(
        noise_size ** (0.5)
    )  # dimensions (height and width) of the square patch array
...
def attack(
    ...
    min_out=0,  # not sure about this one - yayun changed -1 to 0
    ...
    while conf_target > target_prob:
        ...
        Loss = -adv_out[:, target].mean()  # modify here yayun
        Loss.backward()
...
def train(
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        for i in range(new_patch.shape[0]):
            for j in range(new_patch.shape[1]):
                new_patch[i][j] = submatrix(
                    patch[0][j]
                )  # modify here given the last batch of patch will not have 256 samples
...
def test(
    ...
    for batch_idx, (data, labels) in enumerate(test_loader):
        ...
        if patch_type == "circle":
            patch, mask = circle_transform(patch, data_shape, patch_shape, image_size)
        elif patch_type == "square":
            patch, mask = square_transform(patch, data_shape, patch_shape, image_size)
        patch, mask = torch.FloatTensor(patch), torch.FloatTensor(mask)
        patch, mask, data = patch.to(device), mask.to(device), data.to(device)
